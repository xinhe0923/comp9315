...
PageID addToRelation(Reln r, Tuple t)
{
    ...
    //printf("tsig generation finished\n");
    ...
    //freeBits(ppsig);
    //printf("psig generation finished\n");
    ...
    //printf("psig in bsig:");showBits(psig); printf("\n");
    //printf("psigBits(r): %d\n",psigBits(r)); 5584
    for(int i = 0; i < psigBits(r); i++){
        if (bitIsSet(psig, i)) {
            bsigPid = i / maxBsigsPP(r);
            //printf("maxBsigsPP: %d\n",maxBsigsPP(r)); 584
            //printf("bsigPid: %d, nBsigPages: %d \n",bsigPid, nBsigPages(r));
            p = getPage(r->bsigf, bsigPid);
            getBits(p, i % maxBsigsPP(r), slice);
            //printf("%d", i % maxBsigsPP(r)); printf("\n");
            setBit(slice, pid);
            //printf("slice in bsig:");showBits(slice); printf("\n");
            putBits(p, i % maxBsigsPP(r), slice);
            //??addOneItem(p);
            ////////////////////////////////////////////
